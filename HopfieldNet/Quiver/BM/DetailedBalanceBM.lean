...
instance
  ...
  have _ : IsHamiltonian (U:=U) (ζ:=σ) NN := IsHamiltonian_of_EnergySpec' spec
...
lemma single_site_cases
    ...
    (h_ne : s ≠ s') :
    (s.act u = TwoStateNeuralNetwork.ζ_pos (NN := NN) ∧
...
lemma TwoState.EnergySpec'.probPos_flip_pair
  ...
  let ES : TwoState.EnergySpec (NN := NN) :=
    { E := spec.E
      ...
      flip_energy_relation := by intro f' p' s' u'; simpa using spec.flip_energy_relation f' p' s' u' }
  refine ⟨?_, ?_⟩
  ·simpa [ES, f, sPos, sNeg, Δ, sub_eq_add_neg, mul_comm, mul_left_comm, mul_assoc] using
      (ES.probPos_eq_of_energy f p T s u)
  · have hsPos : updPos (NN := NN) sNeg u = sPos := by
      ext v
      by_cases hv : v = u
      · subst hv; simp [sPos, sNeg, updPos, updNeg]
      · simp [sPos, sNeg, updPos, updNeg, hv]
    have hsNeg : updNeg (NN := NN) sNeg u = sNeg := by
      ext v
      by_cases hv : v = u
      · subst hv; simp [sNeg, updNeg]
      · simp [sNeg, updNeg, hv]
    simpa [ES, f, sPos, sNeg, Δ, hsPos, hsNeg, sub_eq_add_neg, mul_comm, mul_left_comm, mul_assoc] using
      (ES.probPos_eq_of_energy f p T sNeg u)

/-- Specialization to the “neg→pos” orientation used in `detailed_balance_neg_pos`.
Here `ΔE = E s' - E s` with `s' = updPos s u`and `s = updNeg s' u` (i.e. `s` carries ζ_neg at `u`,
`s'` carries ζ_pos). -/
lemma flip_prob_neg_pos
    ...
    {s s' : NN.State} {u : U}
    (h_off : ∀ v ≠ u, s.act v = s'.act v)
    (h_neg : s.act u = TwoStateNeuralNetwork.ζ_neg (NN := NN))
    (h_pos : s'.act u = TwoStateNeuralNetwork.ζ_pos (NN := NN)) :
...
lemma detailed_balance_neg_pos
  ...
  have hPpos : 0 < P s := by
    have _ : IsHamiltonian (U:=U) (ζ:=σ) NN :=
