...
lemma RScol_pos_of_diffOnly
  ...
  have hpos_u :
    ...
    -- case on s'.act u
    rcases (TwoStateExclusive.pact_iff (NN:=NN) (a:=s'.act u)).1 (s'.hp u) with hpos' | hneg'
    · -- s'.act u = ζ_pos, hence s.act u = ζ_neg
      have hs_neg :
    ...
    · -- s'.act u = ζ_neg, hence s.act u = ζ_pos
      have hs_pos :
...
lemma exists_single_flip_reduce
  ...
  rcases (TwoStateExclusive.pact_iff (NN:=NN) (a:=s'.act u)).1 (s'.hp u) with hpos | hneg
  · -- Target value is ζ_pos: use updPos
    ...
    · refine And.intro ?hcoord ?hdiffu
      ...
      · -- At u it flips
        have hup : (updPos (NN:=NN) s u).act u = TwoStateNeuralNetwork.ζ_pos (NN:=NN) := by
          simp [updPos]
        -- Need (updPos s u).act u ≠ s.act u
        intro hcontra
        -- Then s.act u = ζ_pos = s'.act u contradicting hneq
    ...
    · -- Show diffSites after update is erase u
      ...
      simp [this]
  · -- Target value is ζ_neg: use updNeg (s'.act u = ζ_neg)
    ...
    · refine And.intro
        ...
        (by
          have hup : (updNeg (NN:=NN) s u).act u = TwoStateNeuralNetwork.ζ_neg (NN:=NN) := by
            simp [updNeg]
          intro hcontra
          have : s.act u = s'.act u := by
...
/-
  MCMC. integration (row-stochastic convention).

  `RScol` is column-stochastic (destination, source). `MCMC.IsStochastic` expects
...
lemma RSrow_isStochastic :
    IsStochastic (RSrow (NN:=NN) (spec:=spec) p T) := by
...
theorem RSrow_exists_unique_stationary_distribution :
    ∃! (π : stdSimplex ℝ NN.State),
      IsStationary (RSrow (NN:=NN) (spec:=spec) p T) π := by
...
- uniqueness of the stationary distribution for `RSrow` (via `MCMC.`).
...
private lemma vecToMeasure_singleton (s : NN.State) :
    vecToMeasure (πBoltzVec (NN:=NN) (spec:=spec) (p:=p) (T:=T)) {s}
  ...
  have h1 :
      (vecToMeasure (πBoltzVec (NN:=NN) (spec:=spec) (p:=p) (T:=T))) S
        = Finset.sum F (fun s : NN.State =>
            (vecToMeasure (πBoltzVec (NN:=NN) (spec:=spec) (p:=p) (T:=T))) {s}) := by
    simpa [F] using
      (measure_eq_sum_singletons
        (NN:=NN)
        (m := vecToMeasure (πBoltzVec (NN:=NN) (spec:=spec) (p:=p) (T:=T)))
        S)
  have h2 :
      (μBoltz (NN:=NN) (spec:=spec) (p:=p) (T:=T)) S
        = Finset.sum F (fun s : NN.State =>
...
theorem πBoltzVec_is_stationary_RSrow :
    IsStationary (RSrow (NN:=NN) (spec:=spec) p T)
      (πBoltzVec (NN:=NN) (spec:=spec) (p:=p) (T:=T)) := by
  have hP : IsStochastic (RSrow (NN:=NN) (spec:=spec) p T) :=
    RSrow_isStochastic (NN:=NN) (spec:=spec) (p:=p) (T:=T)
