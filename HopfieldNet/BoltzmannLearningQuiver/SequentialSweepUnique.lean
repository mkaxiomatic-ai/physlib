...
noncomputable def SSrow (u : U) (p : Params ℝ U) (T : Temperature) :
    Matrix State State ℝ :=
  fun s t => ((singleSiteKernel (NN := NN ℝ U) energySpec p T u) s {t}).toReal

/-- Sequential sweep as row-stochastic matrix product (head site updated first). -/
noncomputable def sweepRowMatrix (p : Params ℝ U) (T : Temperature) :
...
noncomputable def alignCoord (u : U) (p : Params ℝ U) (T : Temperature) (cur tgt : State) : State :=
  if h : cur.act u = tgt.act u then cur
  else
    Classical.choose (exists_single_flip_reduce (NN := NN ℝ U) energySpec p T
      (show u ∈ diffSites (NN := NN ℝ U) cur tgt by simp [diffSites, h]))

private lemma alignCoord_diffOnly {u : U} (p : Params ℝ U) (T : Temperature) {cur tgt : State}
    (hne : cur.act u ≠ tgt.act u) :
    DiffOnly (NN := NN ℝ U) u (alignCoord u p T cur tgt) cur := by
  dsimp [alignCoord]
  split_ifs with heq
  · exact (hne heq).elim
  · exact (Classical.choose_spec (exists_single_flip_reduce (NN := NN ℝ U) energySpec p T
      (show u ∈ diffSites (NN := NN ℝ U) cur tgt by simp [diffSites, heq]))).1

private lemma DiffOnly_symm {u : U} {s s' : State}
    (h : DiffOnly (NN := NN ℝ U) u s s') :
    DiffOnly (NN := NN ℝ U) u s' s :=
  ⟨fun v hv => (h.1 v hv).symm, h.2.symm⟩

private lemma diffOnly_act_eq_target (p : Params ℝ U) (T : Temperature) {u : U} {s cur tgt : State}
    (h : DiffOnly (NN := NN ℝ U) u s cur) (hmem : u ∈ diffSites (NN := NN ℝ U) cur tgt) :
    s.act u = tgt.act u := by
  have hcur_ne : cur.act u ≠ tgt.act u := by simpa [diffSites] using hmem
  rcases (TwoStateExclusive.pact_iff (NN := NN ℝ U) (a := tgt.act u)).1 (tgt.hp u) with htgt_pos | htgt_neg
  · rcases (TwoStateExclusive.pact_iff (NN := NN ℝ U) (a := cur.act u)).1 (cur.hp u) with hcur_pos | hcur_neg
    · exact (hcur_ne (Eq.symm (htgt_pos.trans hcur_pos.symm))).elim
    · rcases (TwoStateExclusive.pact_iff (NN := NN ℝ U) (a := s.act u)).1 (s.hp u) with hs_pos | hs_neg
...
private lemma alignCoord_act_u (u : U) (p : Params ℝ U) (T : Temperature) (cur tgt : State) :
    (alignCoord u p T cur tgt).act u = tgt.act u := by
  dsimp [alignCoord]
  split_ifs with heq
  · exact heq
  · exact diffOnly_act_eq_target p T
      (Classical.choose_spec (exists_single_flip_reduce (NN := NN ℝ U) energySpec p T
        (show u ∈ diffSites (NN := NN ℝ U) cur tgt by simp [diffSites, heq]))).1
      (by simp [diffSites, heq])

private lemma alignCoord_diffSites_sub {u : U} (p : Params ℝ U) (T : Temperature) {cur tgt : State}
    (hne : cur.act u ≠ tgt.act u) :
    (diffSites (NN := NN ℝ U) (alignCoord u p T cur tgt) tgt).card + 1 =
      (diffSites (NN := NN ℝ U) cur tgt).card := by
  dsimp [alignCoord]
  split_ifs with heq
  · exact (hne heq).elim
  · exact (Classical.choose_spec (exists_single_flip_reduce (NN := NN ℝ U) energySpec p T
      (show u ∈ diffSites (NN := NN ℝ U) cur tgt by simp [diffSites, heq]))).2

/-- Single-site row matrix entries are nonnegative. -/
lemma SSrow_nonneg (u : U) (p : Params ℝ U) (T : Temperature) :
    ∀ s t, 0 ≤ SSrow u p T s t := by
  intro s t; dsimp [SSrow]; exact ENNReal.toReal_nonneg

/-- Single-site row matrix is row-stochastic. -/
lemma SSrow_isStochastic (u : U) (p : Params ℝ U) (T : Temperature) :
...
lemma sweepRowMatrix_isStochastic (order : List U) (p : Params ℝ U) (T : Temperature) :
...
private lemma SSrow_pos_of_diffOnly {u : U} {s s' : State} (p : Params ℝ U) (T : Temperature)
    (h : DiffOnly (NN := NN ℝ U) u s s') :
    0 < SSrow u p T s s' := by
  have hK : 0 < HopfieldBoltzmann.Kbm (NN := NN ℝ U) p T u s s' := by
    have hkernel := singleSiteKernel_pos_of_diffOnly (NN := NN ℝ U) energySpec p T h
    rw [singleSiteKernel_singleton_eval (NN := NN ℝ U) energySpec p T u s s'] at hkernel
    exact ENNReal.ofReal_pos.mp hkernel
  have hpos : 0 < ENNReal.ofReal (HopfieldBoltzmann.Kbm (NN := NN ℝ U) p T u s s') :=
    ENNReal.ofReal_pos.mpr hK
  dsimp [SSrow]
  rw [singleSiteKernel_singleton_eval (NN := NN ℝ U) energySpec p T u s s']
  exact ENNReal.toReal_pos (ne_of_gt hpos) (by simp)

private lemma SSrow_diag_pos (u : U) (p : Params ℝ U) (T : Temperature) (s : State) :
    0 < SSrow u p T s s := by
  have hK : 0 < HopfieldBoltzmann.Kbm (NN := NN ℝ U) p T u s s := by
    have hkernel := singleSiteKernel_diag_pos (NN := NN ℝ U) energySpec p T u s
    rw [singleSiteKernel_singleton_eval (NN := NN ℝ U) energySpec p T u s s] at hkernel
    exact ENNReal.ofReal_pos.mp hkernel
  have hpos : 0 < ENNReal.ofReal (HopfieldBoltzmann.Kbm (NN := NN ℝ U) p T u s s) :=
    ENNReal.ofReal_pos.mpr hK
...
private lemma SSrow_alignCoord_pos (u : U) (cur tgt : State) (p : Params ℝ U) (T : Temperature) :
...
lemma sweepRowMatrix_nonneg (order : List U) (p : Params ℝ U) (T : Temperature)
    (s s' : State) :
...
private lemma SSrow_mul_one_sum_eq (w : U) (p : Params ℝ U) (T : Temperature) (mid tgt : State) :
...
private lemma sweepRowMatrix_pos_SSrow_mul_one (w : U) (p : Params ℝ U) (T : Temperature)
...
private lemma sweepRowMatrix_pos_mul_cons (u : U) (us : List U) (p : Params ℝ U) (T : Temperature)
...
private lemma alignCoord_act_u_of_ne {u v : U} (p : Params ℝ U) (T : Temperature)
...
private lemma alignCoord_act_self (v : U) (p : Params ℝ U) (T : Temperature) (cur tgt : State) :
  ...
  · exact diffOnly_act_eq_target p T
      (Classical.choose_spec (exists_single_flip_reduce (NN := NN ℝ U) energySpec p T
        (show v ∈ diffSites (NN := NN ℝ U) cur tgt by simp [diffSites, heq]))).1
      (by simp [diffSites, heq])

private lemma alignCoord_preserves_off {v w : U} (p : Params ℝ U) (T : Temperature)
...
private lemma sweepRowMatrix_pos_one_site_of_partial_align {u w : U} (hwu : w ≠ u)
    (p : Params ℝ U) (T : Temperature) (mid tgt : State) (hu : mid.act u = tgt.act u)
  ...
  · have hdiff : DiffOnly (NN := NN ℝ U) w mid tgt :=
      ⟨fun x hx => by
        by_cases hxu : x = u
        · simpa [hxu] using hu
        · by_cases hxw : x = w
...
private lemma sweepRowMatrix_pos_tail_run {u : U} (p : Params ℝ U) (T : Temperature)
...
private lemma sweepRowMatrix_pos_of_tail {u : U} {us : List U}
    (h : IsFullSweep (u :: us)) (p : Params ℝ U) (T : Temperature)
...
private lemma sweepRowMatrix_pos_of_fullSweep (order : List U) (h : IsFullSweep order)
    (p : Params ℝ U) (T : Temperature) (s s' : State) :
    0 < sweepRowMatrix p T order s s' := by
  induction order generalizing s s' with
  | nil =>
...
noncomputable def negativePhaseVec (temp : Temperature) (p : Params ℝ U) : stdSimplex ℝ State :=
  πBoltzVec (NN := NN ℝ U) energySpec p temp

/-- Entries of `negativePhaseVec` are Boltzmann probabilities $P_{p,T}(s)$. -/
lemma negativePhaseVec_val_eq_P (T : Temperature) (p : Params ℝ U) (s : State) :
    (negativePhaseVec T p).val s =
      HopfieldBoltzmann.P (NN := NN ℝ U) (spec := energySpec) p T s := by
  dsimp [negativePhaseVec, πBoltzVec, μBoltz, modelProbability]
  rw [boltzmann_singleton_eval (NN := NN ℝ U) energySpec p T s]
  exact ENNReal.toReal_ofReal (modelProbability_nonneg T p s)

/-- `SSrow` entries agree with Quiver Boltzmann transition kernel `Kbm`. -/
lemma SSrow_eq_Kbm (u : U) (p : Params ℝ U) (T : Temperature) (s t : State) :
    SSrow u p T s t = HopfieldBoltzmann.Kbm (NN := NN ℝ U) p T u s t := by
  dsimp [SSrow]
  rw [singleSiteKernel_singleton_eval (NN := NN ℝ U) energySpec p T u s t]
  unfold HopfieldBoltzmann.Kbm
  exact ENNReal.toReal_ofReal ENNReal.toReal_nonneg

private lemma SSrow_isReversible_negativePhase (u : U) (T : Temperature) (p : Params ℝ U) :
  ...
  exact detailed_balance (NN := NN ℝ U) energySpec p T u s t
...
private lemma negativePhaseVec_is_stationary_SSrow (u : U) (T : Temperature) (p : Params ℝ U) :
...
theorem sequentialSweep_uniqueStationaryVec_of_fullSweep (order : List U) (h : IsFullSweep order)
    (T : Temperature) (p : Params ℝ U) :
    ∃! v : stdSimplex ℝ State, IsStationary (sweepRowMatrix p T order) v ∧
      vecToMeasure v = negativePhaseMeasure T p := by
  ...
  · dsimp [negativePhaseVec, negativePhaseMeasure]
    exact vecToMeasure_eq_μBoltz (NN := NN ℝ U) energySpec p T
  · intro y hy
...
theorem negativePhaseMeasure_is_sequentialSweep_unique (order : List U) (h : IsFullSweep order)
  ...
  refine ⟨?_, ?_, ?_, ?_⟩
  · exact sequentialSweepKernel_invariant_negativePhase order T p
