...
lemma dobrushinCoeff_nonneg [Nonempty n] (P : Matrix n n ℝ) : 0 ≤ dobrushinCoeff P := by
  ...
  have hmem : 0 ∈ Set.range f := by
    ...
    refine ⟨⟨i0, i0⟩, ?_⟩
    simp [f, rowDist, tvDist, sub_self, abs_zero, Finset.sum_const_zero]
...
lemma tvDist_contract [Nonempty n]
    (P : Matrix n n ℝ) --(hP : MCMC.IsStochastic P)
...
lemma dobrushinCoeff_pow [DecidableEq n] (P : Matrix n n ℝ) [Nonempty n] (hP : IsStochastic P) (k : ℕ) :
