...
import PF.PerronFrobenius.DominancePart2
  ...
  · intro hw_E_r
    ...
    · let w_abs := fun i ↦ |w i|
      ...
      obtain ⟨c_abs, hc_abs_pos, hc_abs_eq⟩ :=
        uniqueness_of_positive_eigenvector_gen hA_irred hr_pos hw_abs_pos
          hv_pos hw_abs_eig hv_eig_f
      let B := 1 + A
  ...
  have h_ker_sq_eq_ker : LinearMap.ker (g ^ 2) = LinearMap.ker g := by
    ...
    · let u := g w
      ...
      have h_dot_g_w : u_star ⬝ᵥ (g w) = 0 := by
        calc u_star ⬝ᵥ (g w) = u_star ⬝ᵥ (f w) - u_star ⬝ᵥ (r • w) := ?_
             _ = (toLin' Aᵀ u_star) ⬝ᵥ w - r * (u_star ⬝ᵥ w) := ?_
             _ = (r • u_star) ⬝ᵥ w - r * (u_star ⬝ᵥ w) := ?_
             _ = r * (u_star ⬝ᵥ w) - r * (u_star ⬝ᵥ w) := ?_
             _ = 0 := ?_
