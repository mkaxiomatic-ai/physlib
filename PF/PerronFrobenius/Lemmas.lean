  ...
  have p_sub' : @Quiver.Path S (letI := Matrix.toQuiver A; inducedQuiver S) i' j' := by
    ...
    have conv : ∀ {a : S},
      ...
      | @cons b c p e ih =>
          ...
          -- Both sides unfold to `0 < A b.val c.val`; defeq, so `exact` suffices.
          exact e
    exact conv p_sub
...
theorem exists_connecting_edge_of_irreducible {A : Matrix n n ℝ} (hA_irred : A.IsIrreducible) {v : n → ℝ}
...
theorem IsPrimitive.of_irreducible_pos_diagonal [Fintype n][Nonempty n] [DecidableEq n] (A : Matrix n n ℝ)
  ...
  · use k, hk_pos
    ...
    have p_loop_len : p_loop.length = 1 := by
      simp_all only [le_refl, lt_add_iff_pos_left, List.Nat.eq_of_le_zero, length_toPath, N, k, p_loop]
...
lemma path_exists_in_support
  ...
  classical
  letI : DecidablePred support := fun x => ‹DecidablePred (· ∈ support)› x
  simpa using
